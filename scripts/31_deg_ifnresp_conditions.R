...
out_dir <- file.path("outputs", "banksy", "deg_ifnresp_conditions")
...
se <- readRDS(obj_file)
message("  ", ncol(se), " cells | ", nrow(se), " genes | class: ", class(se)[1])
...
# =============================================================
# 4. Assign annotation and subset to IFN responsive (Ifit1)
# =============================================================
...
if (!POP_LABEL %in% annotation) {
  stop("Label '", POP_LABEL, "' not found.\n",
       "Present: ", paste(sort(unique(annotation)), collapse = ", "))
}

pop_idx <- which(annotation == POP_LABEL)
se_pop  <- se[, pop_idx]
message("\n  ", POP_LABEL, " cells: ", ncol(se_pop))

sample_vec <- as.character(colData(se_pop)$sample)
cat("\nCells per condition:\n")
print(table(sample_vec))

if (!REF_CONDITION %in% sample_vec) {
  stop("Reference '", REF_CONDITION, "' not found in IFN responsive cells.")
}

valid_comps <- COMPARISONS[sapply(COMPARISONS, function(c) sum(sample_vec == c) >= 10)]
if (length(valid_comps) == 0) stop("No comparison has >= 10 IFN responsive cells.")
...
assay_name <- if ("counts" %in% assayNames(se_pop)) "counts" else assayNames(se_pop)[1]
# suppressWarnings: as.Seurat() renames reducedDim keys — cosmetic only
so            <- suppressWarnings(as.Seurat(se_pop, counts = assay_name, data = NULL))
...
for (comp in valid_comps) {
  ...
  write.csv(mk, file.path(out_dir, paste0("DEG_", slug, ".csv")), row.names = FALSE)
...
cnt_pop          <- assay(se_pop, assay_name)
ngenes_per_cell  <- colSums(cnt_pop > 0)
ncounts_per_cell <- colSums(cnt_pop)
...
# --- Interpretation note ---
note_lines <- c(
  "DEG Interpretation Note \u2014 IFN responsive (Ifit1)",
  "=====================================================",
  "",
  paste0("Generated: ", format(Sys.time(), "%Y-%m-%d %H:%M")),
  "",
  "1. Detection differences across conditions",
  "   The number of genes detected per IFN responsive cell varies across conditions",
  "   (see DEG_detection_context.csv, columns mean_genes_det / median_genes_det).",
  "   This reflects both biological changes (IFN-driven transcriptional activation)",
  "   and technical variation inherent to spatial transcriptomics (MERFISH panel).",
  "",
  "2. Asymmetry in DEG results",
  "   A strong asymmetry (many upregulated genes, few or no downregulated genes)",
  "   can partly reflect higher detection / transcriptome complexity in the LCMV",
  "   conditions relative to mock, rather than purely biological upregulation.",
  "   Genes appearing 'upregulated' may include transcripts more frequently captured",
  "   in conditions with higher overall detection.",
  "",
  "3. IFN responsive population \u2014 special context",
  "   The IFN responsive (Ifit1) cluster is by definition defined by high IFN-driven",
  "   transcription. LCMV-infected conditions will have more cells in this cluster",
  "   and higher overall expression of IFN-stimulated genes. This makes upward DEG",
  "   asymmetry expected biologically, but also harder to disentangle from detection",
  "   differences.",
  "",
  "4. LCMV_3wpi vs mock \u2014 special caution",
  ...
  "",
  "5. Recommended interpretation approach",
  ...
  "   - Cross-reference IFN-signature genes (e.g. Ifit1, Ifit3, Mx1, Oas1) which",
  "     are expected to be genuinely upregulated during active infection.",
  "",
...
p_det <- ggplot(detect_cells_df,
  ...
  labs(
    title = "IFN responsive (Ifit1) \u2014 detected genes per cell",
    x     = NULL,
...
CairoPDF(file.path(out_dir, "fig_detection_boxplot_ifnresp.pdf"), width = 5, height = 4)
print(p_det)
dev.off()
CairoJPEG(file.path(out_dir, "fig_detection_boxplot_ifnresp.jpg"),
          width = 5 * 150, height = 4 * 150, res = 150)
print(p_det)
dev.off()
message("Saved: fig_detection_boxplot_ifnresp.pdf / .jpg")
...
make_volcano <- function(mk, comp) {
  ...
  p <- ggplot(mk, aes(x = x_plot, y = y_plot,
    ...
    labs(
      title    = paste0("IFN responsive (Ifit1): ", label, " vs ", ref_l),
...
cat("  fig_detection_boxplot_ifnresp.pdf / .jpg\n")
